...
namespace LevelGenerator.Scripts
{
    public class LevelGenerator : MonoBehaviour
    {
        ...
        IEnumerator WaitABit()
        {
            ...
            // Stuff that was in Start() before

            if (Seed != 0)
            {
                RandomService.SetSeed(Seed); // Setting LevelGenerator's separate seed for its custom scripts for randomness
                //Random.InitState(Seed); // Setting Unity's/C#'s seed for randomness
            }
            else
            {
